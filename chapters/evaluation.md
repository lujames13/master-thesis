...
模擬環境:本實驗在離線模擬環境中進行,未部署於真實區塊鏈網絡。因此,實際的網絡延遲、共識延遲、以及鏈上存儲成本等因素尚未通過實際測量驗證。未來工作將在 Ethereum 測試網或類似環境中部署原型系統,測量實際的系統性能。

攻擊場景簡化:本實驗主要驗證了「權益佔領攻擊」這一核心威脅,但未涵蓋所有可能的攻擊變種。例如,攻擊者可能採用更隱蔽的策略,如逐步降低模型質量而非完全拒絕誠實更新,以避免觸發挑戰。未來研究需要探討更複雜的攻擊場景,並評估挑戰機制在這些場景下的有效性。

規模限制:實驗中的節點數量 (100 個) 和訓練輪數 (100 輪) 相對較小,未測試極大規模場景。在實際應用中,聯邦學習系統可能包含數千甚至數萬個節點,訓練可能持續數百輪。未來工作需要在更大規模的環境中驗證系統的可擴展性。

經濟參數調優:罰沒比例、獎勵分配等經濟參數的設定對系統的激勵相容性有重要影響。本實驗採用了固定的參數配置,但在實際部署中,這些參數可能需要根據具體應用場景進行調優。未來研究需要探討如何設計自適應的經濟機制,以適應不同的威脅環境。

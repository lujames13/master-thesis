...
#### D.1.1 Core Mechanism (0.2頁)

零知識機器學習(zkML)將密碼學中的零知識證明技術應用於ML驗證:證明者可向驗證者證明「某ML計算被正確執行」,而無需透露計算過程或輸入數據本身[TODO: zkML-overview]。這種特性使zkML在隱私保護和信任最小化場景中具有獨特價值。

**證明系統**: 當前zkML主要採用兩類系統:(1)**zk-SNARK**——證明小(數百bytes至KB)、驗證快(毫秒級),但需可信設置,代表為Groth16[TODO: Groth16]和PLONK[TODO: PLONK];(2)**zk-STARK**——無需可信設置、後量子安全,但證明較大(數十KB至MB),代表為基於FRI的STARKs[TODO: STARKs-FRI]。

**核心挑戰:算術化**: zkML的關鍵技術挑戰是將神經網絡計算「算術化」為算術電路或R1CS約束系統。需將高層ML操作(矩陣乘法、激活函數、聚合算法)分解為基本算術運算(加法、乘法)和有限域上的約束。例如,ReLU激活函數需轉換為涉及比較的電路,而比較需進一步分解為位運算和算術運算組合。

**數學級安全但代價高昂**: zkML的優勢是安全性僅依賴密碼學假設(如離散對數困難性),不需要信任假設——即使所有參與者惡意,錯誤計算也無法生成有效證明。這種「trustless」特性在無許可公鏈或極高安全要求場景(如金融審計、醫療合規)中有價值。然而,算術化過程導致電路規模爆炸,證明生成開銷極大,這將在下節深入批判。
...
**硬體加速的局限性**: 雖然近期研究嘗試使用GPU加速zkML證明生成,但由於密碼學操作(如模運算、多項式求值)與ML計算(如矩陣乘法)的特性差異,GPU加速的效果有限。Ingonyama等公司的研究顯示,GPU加速可將證明生成時間降低約10-20倍[TODO: Ingonyama-GPU],但這仍意味著ResNet50的證明生成需要約3-5分鐘,相較於2秒的原生計算仍慢數個數量級。此外,GPU加速zkML證明生成需要專用硬體(如Ingonyama的FPGA加速卡),部署成本高昂,進一步限制了其實用性。

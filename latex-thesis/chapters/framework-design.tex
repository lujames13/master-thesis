...
\begin{itemize}
    \item \textbf{決策依據}：若選擇回滾模型 (Revert)，將導致基於該惡意模型後續訓練的所有輪次失效，造成巨大的算力浪費。系統選擇承受單次攻擊的代價以換取高執行效率。
    \item \textbf{處理方式}：
    \begin{itemize}
        \item \textbf{執行懲罰}：罰沒惡意委員會成員與聚合者的全額質押金，並將其分配給挑戰者作為獎勵。
        \item \textbf{模型處理}：保留該次更新。系統依靠 FL 演算法自身的自我修正能力，由後續誠實更新逐步覆蓋其影響。
    \end{itemize}
    \item \textbf{威懾力}：雖然攻擊者成功注入一次毒化更新，但其付出了巨額資金損失且被踢出網路，中斷了 PCCA 的權益指數增長鏈。
\end{itemize}

...
在主流共識協議（如 Casper FFG \cite{buterin2017casper}、Tendermint \cite{tendermint}、Cosmos \cite{kwon2016cosmos} 或 Polkadot \cite{wood2016polkadot}）中，驗證者必須存入押金。若驗證者違反協定規則（例如雙重投票），其部分或全部押金將被系統自動罰沒。這種機制確保了系統具有「可問責性」（Accountability），即任何破壞安全性的行為都能被識別並追究經濟責任 \cite{chiu2018incentive}。

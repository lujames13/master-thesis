\documentclass[12pt, a4paper]{ntut-report}
\usepackage[dvips,xetex]{graphicx}
\usepackage{ifpdf,mla}% <-- mla.sty requires ifpdf.sty, but (perversely) doesn't load it
%\usepackage{fontspec}
\usepackage{geometry}
\usepackage{lipsum}
\usepackage{xeCJK}
\usepackage{wallpaper}
\usepackage{pdfpages}
\usepackage{indentfirst}
\usepackage{setspace}
\usepackage{hyperref}
\usepackage{zhnumber}
\usepackage{titlesec}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsthm}

\newtheorem{theorem}{定理}[chapter]
\newtheorem{lemma}[theorem]{引理}
\newtheorem{corollary}[theorem]{推論}
\newtheorem{definition}[theorem]{定義}
\newtheorem{assumption}[theorem]{假設}
\newtheorem{remark}[theorem]{備註}
\renewcommand{\proofname}{證明}
\usepackage{xurl}
\usepackage[backend=biber,sorting=none]{biblatex}
\setcounter{biburllcpenalty}{9000}
\setcounter{biburlucpenalty}{9000}
\setcounter{biburlnumpenalty}{9000}
\usepackage{tabularx}
\usepackage{multirow}
\usepackage{csquotes}
\usepackage{subcaption}
\usepackage{algorithm}
\usepackage{algpseudocode}

\MakeAutoQuote{“}{”}
\MakeOuterQuote{"}

\captionsetup{justification=centering,singlelinecheck=false,font={normalsize,stretch=2}}
% \captionsetup[sub]{justification=centering,singlelinecheck=false,font={normalsize,stretch=2}}


\renewcommand{\baselinestretch}{1.5}
\xeCJKsetup{AutoFakeBold=true, AutoFakeSlant=true}
\setCJKmainfont{DFKai-SB.ttf} % 中文字體
\setCJKmonofont{DFKai-SB.ttf} % 設定等寬中文字體，解決 \CJKttdefault 警告

\setmainfont{FreeSerif} % 英文字體 (Replacement for missing Times New Roman)
\geometry{a4paper,top=4.5cm,bottom=2.5cm,left=2.5cm,right=2.5cm,footskip=1.0cm} % 修正頁面設定，確保頁碼可見

\pagestyle{plain} % 頁碼
\pagenumbering{arabic}
\addbibresource{reference.bib}

\input{ntut-labels.tex}

\begin{document}

% 封面、不用浮水印 Cover without a watermark
\include{page/titlepage.tex}
% 留白頁、不用浮水印 Blank page without a watermark
\include{page/blankpage.tex}

% 新增浮水印 Add Watermarks
\CenterWallPaper{.64}{ntut-logo.png}

% 封面、要浮水印 Cover with a watermark
\input{page/titlepage.tex}
% 「學位論文口試委員會審定書」掃描檔，請檢附完整簽名之審定書掃描檔
% Please add "Oral Defense Committee Signature Form" with complete signatures.
% \includepdf[pages=-]{static-page/signpage.pdf}

\pagenumbering{roman}

% 摘要頁（中文）Abstract (Chinese)
% If you don’t have Chinese abstract, please delete this one.
\include{page/abstract.tex}
% 摘要頁（英文）Abstract (English)
\include{page/abstract-en.tex}
% 鳴謝 Acknowledgements
\include{page/thanks.tex}
% 目錄、表目錄與圖目錄 Table of Contents, List of tables, List of figures
\input{page/table-of-content.tex}
 
\pagenumbering{arabic}

% 章節一 Chapter 1
\include{chapters/introduction.tex}
% 章節二 Background and Related Work
\include{chapters/background-and-related-work.tex}

% 章節三 Threat Model
\include{chapters/threat-model.tex}
% 章節四 Framework Design
\include{chapters/framework-design.tex}
% 章節五 Evaluation
\include{chapters/evaluation.tex}
% 章節六 Conclusion
\include{chapters/conclusion.tex}
% 新增你自己的章節... Add chapters here

% 參考文獻，請在段落上隨意註解，你同時需要 reference.bib
\setlength{\emergencystretch}{3em}
\include{page/reference}

\end{document}

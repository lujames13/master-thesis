...
1. **計算負荷分散**：單一聚合器不需承擔所有計算壓力，計算負擔降低 N 倍
...
假設聯邦學習需要進行 R 個訓練輪次，系統有 N 個聚合器和 V 個驗證者。在拜占庭容錯的條件下，系統最多可容忍 $f = \lfloor \frac{V-1}{3} \rfloor$ 個惡意驗證者。

#### (1) 傳統 PBFT 方案的計算負擔

傳統 PBFT 方案（如 FLCoin [9]）中，每個驗證者都需要參與每輪聚合的驗證：
- **每個驗證者的聚合運算次數**：R 次
- **系統總聚合運算次數**：$R \times V$ 次
...
若有惡意驗證者在每個回合都惡意發起挑戰（犧牲其質押），系統將在每回合啟動PBFT共識：
- 聚合器：仍為 $\frac{R}{N}$ 次（輪替不變）
- 驗證者：$R \times V$ 次（每輪 V 個驗證者都驗證）
- 系統總計：$R \times (1 + V)$ 次
...
在實務中，設挑戰發生概率為 p（通常 $p \ll 1$），則：
- 聚合器：$\frac{R}{N}$ 次
- 驗證者：$p \times R \times V$ 次（僅挑戰輪驗證）
- 系統總計：$R \times (1 + p \times V)$ 次

當 p 很小時（通常 $p < 0.1$），實際工作負載接近最樂觀情況。

#### (3) 計算複雜度對比

| 方案 | 單個節點計算量 | 系統總計算量 |
|------|---------------|-------------|
| 傳統 PBFT | R 次/驗證者 | $R \times V$ |
| 本研究（正常） | $\frac{R}{N}$ 次/聚合器 | R |
| 本研究（挑戰率 p） | $\frac{R}{N}$ 次/聚合器<br>+ $p \times R$ 次/驗證者 | $R \times (1 + p \times V)$ |

#### (4) 計算效率提升

**單個節點負擔降低**（聚合器 vs 驗證者）：
$$\text{提升倍數} = \frac{R}{\frac{R}{N}} = N \text{ 倍}$$
...
- **混合模式**：$O(\frac{R}{N}(1 + p \times V))$ 平均每節點

當 $N \gg 1$ 且 p 為小常數時，混合模式的複雜度主要由第一項決定，實現了接近樂觀模式的效率。
...
**每輪通訊複雜度**：$O(n^2)$，其中 n 為驗證者數量

**消息交換過程**：
1. Pre-Prepare 階段：1 條消息（主節點廣播）
2. Prepare 階段：$n \times (n-1)$ 條消息（每個節點廣播給其他節點）
3. Commit 階段：$n \times (n-1)$ 條消息（每個節點再次廣播）
...
|------|---------|---------|------------------|
| 傳統 PBFT | $O(n^2)$ 每輪 | $O(n^2)$ 每輪 | $O(n^2)$ |
| 本研究 | O(1) 每輪 | $O(n^2)$ 每輪 | $O(1) \times 0.95 + O(n^2) \times 0.05 \approx O(1)$ |
...
- 提升倍數：約 $n^2$ ÷ 1 = **441 倍**（正常情況）

**可擴展性優勢**：
- 傳統 PBFT 的可擴展性受限於 $O(n^2)$ 通訊複雜度，實際部署通常限制在 10-20 個驗證者
...
| 維度 | 傳統 PBFT | 本研究 | 提升倍數 |
|------|----------|--------|---------|
| **聚合器計算負擔** | R 次/節點 | $\frac{R}{N}$ 次/節點 | N 倍 |
| **通訊開銷（正常）** | $O(n^2)$ 每輪 | O(1) 每輪 | $n^2$ 倍 |
| **系統總計算量**（p=0.05） | $R \times V$ | $R \times (1+p \times V)$ | $\frac{V}{1+p \times V}$ 倍 |
...
傳統 PBFT 因 $O(n^2)$ 通訊複雜度，實務上難以支援超過 20 個驗證者。本研究通過樂觀通過機制，使系統可以支援更多驗證者（例如 50-100 個）而不顯著影響效率。
...
- 正常情況（95%）：O(1) 通訊 + $\frac{R}{N}$ 計算
- 威脅情況（5%）：$O(n^2)$ 通訊 + $R \times V$ 計算

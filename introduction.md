...
為解決上述問題，區塊鏈技術的引入為聯邦學習提供了新的發展方向。區塊鏈的去中心化特性、不可篡改性和智能合約功能，為建立可信的聯邦學習環境提供了技術基礎。然而，現有的區塊鏈聯邦學習方案多數仍採用傳統的拜占庭容錯（PBFT）共識機制，雖然提供了強安全性保障，卻因其O(n²)的通信複雜度導致效能瓶頸。
...
現有方案在追求安全性的同時，往往犧牲了系統效能。採用完整PBFT共識的方案雖然能夠抵禦拜占庭錯誤，但其O(n²)的通信複雜度使得系統在參與者數量增加時面臨嚴重的擴展性問題。相對地，僅使用區塊鏈作為不可變記錄的方案雖然效能較佳，但缺乏對惡意聚合器的有效防護機制。

### 1.2.2 資源利用效率不足

傳統PBFT共識要求所有參與者在每一輪都進行完整的共識過程，這導致大量不必要的計算和通信開銷。在大多數情況下，聚合過程是誠實的，完整的拜占庭共識顯得過於繁重。
...
1. **設計混合式安全機制**：開發一個結合Optimistic Rollup樂觀原理與PBFT拜占庭容錯的安全聚合機制，在樂觀情況下達到O(n)的效能，同時在面對挑戰時提供O(n²)等級的拜占庭容錯保護。
...
- **樂觀模式**：在正常情況下假設聚合器誠實，使用高效的輪詢選擇機制（O(n)複雜度）
- **挑戰模式**：當檢測到可疑行為時，啟動PBFT共識驗證（O(n²)複雜度）
- **模式切換**：根據系統狀態動態切換安全等級，實現效能與安全的最佳平衡
...
3. **複雜度分析**：證明了混合式方法在樂觀情況下達到O(n)複雜度，而在挑戰情況下提供O(n²)的拜占庭容錯保護，並分析了兩種模式的切換條件。
